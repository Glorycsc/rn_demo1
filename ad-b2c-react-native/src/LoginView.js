...
export default class LoginView extends PureComponent {
  ...
  async onNavigationStateChangeAsync(navState) {
    ...
    if (url.indexOf(this.props.redirectURI) == 0) {
  ...
  onShouldStartLoadWithRequest(navState) {
    ...
    if (
      ...
      console.log('stop loading because requesttype is :' + result.requestType + '\r\nand url is ' + navState.url);
      const { url } = navState;
      const { uri: stateUri } = this.state;
      if (url.indexOf(this.props.redirectURI) == 0) {
        console.log('redirect url called, capturing JWT token');
        this._handleFlowResultAsync(result, stateUri);
      }
